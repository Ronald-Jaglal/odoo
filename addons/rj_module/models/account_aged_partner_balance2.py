...
class ReportAccountAgedPartner2(models.AbstractModel):
    _name = "account.aged.partner2"
    _description = "Aged Partner Balances 2"
    ...
    report_date = fields.Date(groupInvoicerator='max', string='Ar2 Date')
    ...
    period1 = fields.Monetary(string='1 - 14')
    period2 = fields.Monetary(string='15 - 30')
    period3 = fields.Monetary(string='31 - 45')
    period4 = fields.Monetary(string='46 - 60')
    period5 = fields.Monetary(string='61 - 90')
    period6 = fields.Monetary(string='91 & Older')
    amount_currency = fields.Monetary(currency_field='currency_id')

    @api.model
    def _get_templates(self):
        # OVERRIDE
        templates = super(ReportAccountAgedPartner2, self)._get_templates()
    ...
    def _get_query_period_table(self, options):
        ...
        01 - 14             | 2019-01-09    | 2018-12-27
        15 - 30             | 2018-12-27    | 2018-12-12
        31 - 45             | 2018-10-27    | 2018-11-12
        46 - 60             | 2018-09-12    | 2018-10-27
        61 - 90             | 2018-08-27    | 2018-09-12
        91 and older        | 2018-08-12    | 2018-08-27
        Older               |               | 2018-08-12
        ...
        period_values = [
            (False,                  date_str),
            (minus_days(date, 1),    minus_days(date, 14)),
            (minus_days(date, 15),   minus_days(date, 30)),
            (minus_days(date, 31),   minus_days(date, 45)),
            (minus_days(date, 46),   minus_days(date, 60)),
            (minus_days(date, 61),   minus_days(date, 90)),
            (minus_days(date, 91),  False),
        ]
    ...
    def _get_sql(self):
        ...
        params = {
            ...
            'sign': 1 if options['filter_account_type'] == 'receivable2' else -1,
    ...
    def _get_column_details(self, options):
        columns = [
            ...
            self._custom_column(  # Avoid doing twice the sub-select in the view
                ...
                getter=(lambda v: v['period0'] + v['period1'] + v['period2'] + v['period3'] + v['period4'] + v['period5'] + v['period6']),
                sortable=True,
            ),
        ]
    ...
    def _show_line(self, report_dict, value_dict, current, options):
        ...
        all_zero = all(
            ...
            for f in ['period0', 'period1', 'period2', 'period3', 'period4', 'period5','period6']
    ...
    def _format_total_line(self, res, value_dict, options):
        res['name'] = _('Total')
        res['colspan'] = len(self._get_column_details(options)) - 8
        res['columns'] = res['columns'][res['colspan']-1:]


class ReportAccountAgedReceivable(models.Model):
    _name = "account.aged.receivable2"
    _description = "Aged Receivable 2"
    _inherit = "account.aged.partner2"
    _auto = False

    def _get_options(self, previous_options=None):
        # OVERRIDE
        options = super(ReportAccountAgedReceivable2, self)._get_options(previous_options=previous_options)
        options['filter_account_type'] = 'receivable2'
        return options

    @api.model
    def _get_report_name(self):
        return _("Aged Receivable custom")

    @api.model
    def _get_templates(self):
        # OVERRIDE
        templates = super(ReportAccountAgedReceivable2, self)._get_templates()
        templates['line_template'] = 'account_reports.line_template_aged_receivable_report2'
        return templates
